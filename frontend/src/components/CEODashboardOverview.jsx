      ...
      <div className="space-y-6 animate-pulse py-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {[1, 2, 3, 4, 5, 6, 7, 8].map(i => (
            <div key={i} className="h-36 bg-[#14151c] rounded-2xl border border-white/5" />
          ))}
        </div>
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-gradient-to-r from-[#181924] via-[#151620] to-[#12131a] border border-amber-500/20 rounded-3xl p-5 sm:p-6 shadow-2xl relative overflow-hidden backdrop-blur-xl">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 relative z-10">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Complete real-time executive dashboard across sales, finance, operations, customers, team and reporting.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <button
              ...
              disabled={refreshing}
              className="p-2 bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white border border-white/10 rounded-2xl text-xs font-semibold transition cursor-pointer"
        ...
        <div className="flex items-center gap-3 text-[11px] bg-black/40 border border-white/10 px-3.5 py-2.5 rounded-xl text-gray-400 flex-wrap mt-4">
          ...
          <span className="flex items-center gap-1.5 text-emerald-400 font-semibold bg-emerald-500/10 px-2.5 py-0.5 rounded-lg border border-emerald-500/20">
            ...
            Verified Live DB: Invoices, Collections, Receivables Aging, Quotes, Visits, Rep Slabs
          </span>
          <span className="flex items-center gap-1.5 text-amber-300 font-semibold bg-amber-500/10 px-2.5 py-0.5 rounded-lg border border-amber-500/20">
            ...
            Fabricated / Modeled: Gross Profit (52% COGS), Net Profit (24% Opex), Weighted Leads — Formulas Displayed on Cards
          </span>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between px-1">
          ...
          <span className="text-xs text-gray-400 font-medium">Real-Time Aggregations & Formulas</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          {executiveKPIs.map((kpi) => {
            ...
            const isFabricated = kpi.isFabricated || kpi.id === 'kpi_gp' || kpi.id === 'kpi_np' || kpi.id === 'kpi_pipeline';
            ...
            return (
              <motion.div
                ...
                className={`p-5 rounded-2xl border transition-all relative overflow-hidden flex flex-col justify-between ${isRevenue
              ...
              >
                <div>
                  <div className="flex items-center justify-between gap-2 text-xs font-bold text-gray-300">
                    <span className="text-gray-300 font-bold">{kpi.title}</span>
                    <div className="flex items-center gap-1.5 shrink-0">
                      {isFabricated ? (
                        ...
                        <span className="text-[9px] bg-emerald-500/15 text-emerald-400 border border-emerald-500/25 px-1.5 py-0.5 rounded font-black tracking-wide uppercase">
                          Live DB
                        </span>
                      )}
                      <span className={`text-[11px] px-2 py-0.5 rounded-full font-black ${kpi.trend === 'up'
                ...
                <div className="mt-4 pt-3 border-t border-white/5 space-y-1">
                  ...
                  {isFabricated && (
                    <div className="text-[10px] font-mono text-amber-300/90 bg-amber-500/10 px-2 py-1 rounded-lg border border-amber-500/20 mt-1">
                      {kpi.id === 'kpi_gp' && 'Formula: Revenue - (Revenue × 52% COGS)'}
                      {kpi.id === 'kpi_np' && 'Formula: Gross Profit - (Revenue × 24% Opex)'}
                      {kpi.id === 'kpi_pipeline' && 'Formula: Live Quotes + (Hot × ₹1.2L + Warm × ₹65K)'}
                    </div>
                  )}
                </div>
              </motion.div>
      ...
      {/* ── 2. DETAILED DOMAIN HUBS (ALL 10 SECTIONS IN COMMAND HUB) ───────── */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-1">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[9px] bg-amber-500/20 text-amber-300 px-1.5 py-0.5 rounded font-black uppercase">Fabricated Model</span>
            </div>
            <div className="text-xl font-black text-white">₹{(sales.totalPipelineValue / 100000).toFixed(2)}L</div>
            <p className="text-[10px] font-mono text-amber-300/80">Formula: Quotes + (Hot × ₹1.2L + Warm × ₹65K)</p>
          </div>

          <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-1">
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-400 font-medium">Active Quotations Value</span>
              <span className="text-[9px] bg-emerald-500/20 text-emerald-400 px-1.5 py-0.5 rounded font-black uppercase">Live DB</span>
            </div>
      ...
      <div className="bg-[#14151c] border border-white/10 rounded-3xl p-6 space-y-6 shadow-xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            ...
            <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-3">
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-400">Total Billed Invoices (Live DB)</span>
                <span className="font-bold text-white">₹{(finance.totalInvoiced / 100000).toFixed(2)}L</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-400">Cash Inflow Cleared (Live DB)</span>
                <span className="font-bold text-emerald-400">₹{(finance.totalCollected / 100000).toFixed(2)}L</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-400">Pending Receivables (Live DB)</span>
              ...
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-400">Est. COGS (Fabricated Benchmark)</span>
                <span className="font-bold text-amber-300">₹{(finance.estimatedCOGS / 100000).toFixed(2)}L (Formula: Billed × 52%)</span>
              </div>
            </div>
          </div>
          ...
          {/* Aging Buckets */}
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-gray-300 uppercase tracking-wider">Receivables Aging Brackets (Live DB)</h4>
            <div className="space-y-2">
              <div className="bg-black/30 border border-white/5 p-3 rounded-xl flex items-center justify-between">
                <div>
                  <div className="text-xs font-bold text-white">0 - 30 Days (Current)</div>
                  <div className="text-[10px] text-gray-400">{finance.agingBuckets?.current?.count || 0} Invoices</div>
                </div>
                <div className="text-xs font-black text-emerald-400">
                  ₹{((finance.agingBuckets?.current?.amount || 0) / 100000).toFixed(2)}L
                </div>
              </div>

              <div className="bg-black/30 border border-white/5 p-3 rounded-xl flex items-center justify-between">
                <div>
                  <div className="text-xs font-bold text-white">31 - 60 Days</div>
                  <div className="text-[10px] text-gray-400">{finance.agingBuckets?.days31_60?.count || 0} Invoices</div>
                </div>
                <div className="text-xs font-black text-amber-400">
                  ₹{((finance.agingBuckets?.days31_60?.amount || 0) / 100000).toFixed(2)}L
                </div>
              </div>

              <div className="bg-black/30 border border-white/5 p-3 rounded-xl flex items-center justify-between">
                <div>
                  <div className="text-xs font-bold text-white">61 - 90 Days</div>
                  <div className="text-[10px] text-gray-400">{finance.agingBuckets?.days61_90?.count || 0} Invoices</div>
                </div>
                <div className="text-xs font-black text-orange-400">
                  ₹{((finance.agingBuckets?.days61_90?.amount || 0) / 100000).toFixed(2)}L
                </div>
              </div>

              <div className="bg-black/30 border border-rose-500/20 p-3 rounded-xl flex items-center justify-between">
                <div>
                  <div className="text-xs font-bold text-rose-400">90+ Days (High Risk)</div>
                  <div className="text-[10px] text-rose-400/80">{finance.agingBuckets?.days90Plus?.count || 0} Invoices</div>
                </div>
                <div className="text-xs font-black text-rose-400">
                  ₹{((finance.agingBuckets?.days90Plus?.amount || 0) / 100000).toFixed(2)}L
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Recent Collections */}
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-gray-300 uppercase tracking-wider">Latest Payment Settlements (Live DB)</h4>
            <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-2.5">
              {finance.recentPayments.length === 0 ? (
                <div className="text-xs text-gray-500 text-center py-4">No recent payments logged</div>
              ) : (
                finance.recentPayments.map(p => (
                  <div key={p.id} className="flex items-center justify-between text-xs py-1 border-b border-white/5 last:border-0">
                    <div>
                      <div className="font-bold text-white">{p.school_name || 'Direct School Payment'}</div>
                      <span className="text-[10px] text-gray-500">{p.payment_date} • {p.payment_mode}</span>
                    </div>
                    <span className="font-black text-emerald-400">₹{Number(p.amount_paid).toLocaleString('en-IN')}</span>
                  </div>
                ))
              )}
            </div>
      ...
      {/* ── SECTION 3: OPERATIONS & FIELD DISPATCH ───────────────────────── */}
      <div className="bg-[#14151c] border border-white/10 rounded-3xl p-6 space-y-6 shadow-xl">
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-black text-white">Operations, Delivery & Follow-Up Deadlines</h3>
              <p className="text-xs text-gray-400">Sample dispatches, quotation turns, customer follow-up actions & delays</p>
            </div>
          </div>
          <button
          ...
          >
            <span>Central Follow-Ups Hub</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-1">
            <span className="text-xs text-gray-400 font-medium">Orders In Progress</span>
            <div className="text-xl font-black text-white">{operations.totalOrders}</div>
            <p className="text-[11px] text-emerald-400">Won client commitments (Live DB)</p>
          </div>

          <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-1">
            <span className="text-xs text-gray-400 font-medium">Samples Dispatched</span>
            <div className="text-xl font-black text-cyan-400">{operations.sampleSentCount}</div>
            <p className="text-[11px] text-gray-500">Fabric & fit samples at schools (Live DB)</p>
          </div>

          <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-1">
            <span className="text-xs text-gray-400 font-medium">Quotes Under Review</span>
            <div className="text-xl font-black text-amber-400">{operations.quoteGivenCount}</div>
            <p className="text-[11px] text-gray-500">Awaiting school signoff (Live DB)</p>
          </div>

          <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-1">
            <span className="text-xs text-gray-400 font-medium">Follow-Up Action Items</span>
            <div className="text-xl font-black text-rose-400">{operations.overdueFollowUpsCount}</div>
            <p className="text-[11px] text-gray-500">Urgent field follow-ups (Live DB)</p>
          </div>
        </div>
        ...
        {operations.overdueFollowUps.length > 0 && (
          <div className="space-y-3 pt-2">
            <h4 className="text-xs font-bold text-gray-300 uppercase tracking-wider flex items-center gap-2">
              ...
              Upcoming & Priority Follow-Up Commitments (Live DB)
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {operations.overdueFollowUps.map(f => (
                <div key={f.id} className="bg-black/30 border border-white/5 p-3.5 rounded-2xl flex items-center justify-between text-xs">
                  <div>
      ...
      <div className="bg-[#14151c] border border-white/10 rounded-3xl p-6 space-y-6 shadow-xl">
        ...
        <div className="space-y-3">
          <h4 className="text-xs font-bold text-gray-300 uppercase tracking-wider">Field Demand Frequency by Product (Live DB Visit Logs)</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {inventory.demandDistribution.map(item => {
              ...
              return (
                <div key={item.product} className="bg-black/30 border border-white/5 p-3.5 rounded-xl flex items-center justify-between gap-4">
                  <div className="w-48 text-xs font-bold text-white truncate">{item.product}</div>
                  <div className="flex-1 bg-white/10 h-2 rounded-full overflow-hidden">
                    <div className="bg-gradient-to-r from-amber-400 to-purple-400 h-full rounded-full" style={{ width: `${pct}%` }} />
                  </div>
                  <span className="text-xs font-black text-amber-400 w-20 text-right">{item.count} Inquiries</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            ...
            <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-3">
              ...
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-400">Canvassed / Visited (Live DB)</span>
                <span className="font-bold text-amber-400">{customers.visitedCount} Schools</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-400">Converted Client Accounts (Live DB)</span>
          ...
          <div className="lg:col-span-2 space-y-3">
            <h4 className="text-xs font-bold text-gray-300 uppercase tracking-wider">Top Revenue Contributing Institutions (Live DB)</h4>
      ...
      <div className="bg-[#14151c] border border-white/10 rounded-3xl p-6 space-y-6 shadow-xl">
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          ...
          <span className="text-[10px] font-bold text-amber-300 bg-amber-500/15 border border-amber-500/30 px-3 py-1 rounded-full uppercase tracking-wide">
            Fabricated Formula: 52% Sourcing COGS
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-1">
            ...
            <p className="text-[10px] font-mono text-amber-300/80">Formula: Total Invoiced × 0.52</p>
          </div>
          ...
          <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-1">
            ...
            <p className="text-[11px] text-gray-500">Fabricated benchmark assumption</p>
          </div>
          ...
          <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-1">
            ...
            <p className="text-[10px] font-mono text-emerald-400/80">Formula: 100% - 52% COGS</p>
          </div>
      ...
      <div className="bg-[#14151c] border border-white/10 rounded-3xl p-6 space-y-6 shadow-xl">
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          ...
          <span className="text-[10px] font-bold text-amber-300 bg-amber-500/15 border border-amber-500/30 px-3 py-1 rounded-full uppercase tracking-wide">
            Fabricated Benchmarks: CAC & ROI Models
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-gray-300 uppercase tracking-wider">Field Lead Qualification Mix (Live DB Visits)</h4>
            <div className="bg-black/30 border border-white/5 p-4 rounded-2xl space-y-3">
              <div className="flex items-center justify-between text-xs">
              ...
              <div className="flex items-center justify-between text-xs pt-2 border-t border-white/5">
                <span className="text-gray-400">Total School Visits Logged (Live DB)</span>
          ...
          {/* Campaign Drives */}
          <div className="lg:col-span-2 space-y-3">
            <h4 className="text-xs font-bold text-gray-300 uppercase tracking-wider">Outreach Campaigns (Fabricated Benchmarks)</h4>
              ...
              {(marketing.campaigns || []).slice(0, 4).map(c => (
                <div key={c.id} className="bg-black/30 border border-white/5 p-3.5 rounded-2xl space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-black text-emerald-400">{c.roi || '5.2x ROI (Model)'}</span>
                  </div>
